...
2020.05.24 by 内联后勤副紫红

2021.01.09 updated 去掉cv2和numpy依赖
...
2. 调整文字的字体、字号、位置、颜色，照片的位置。将代码第18行的变量`is_designing`的值改为1即可进入设计模式，调整第16~第63行的参数即可调节上述内容。运行代码，会输出字块所占的实际像素数，还会在当前目录下生成名为`a_designing_IDcard.jpg`的样张，可以观察它的效果不断修改参数直到满意为止。然后把`is_designing`的值改回0，重新运行即可。
